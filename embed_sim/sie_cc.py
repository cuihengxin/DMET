...
##### for AODMET ###### 
def sie_rohf_uccsd_t_ao(mol, imp_idx, title, full_cc=False, x2c=True, frozen=False):
    from pyscf import cc, mp, lib, scf, df
    if x2c == True:
        mf = scf.rohf.ROHF(mol).x2c().density_fit()
    else:
...
def sie_rohf_uccsd_t_cl_ao(mol, imp_idx, title, full_cc=False, shell=None, atoms_A=None, proj_bas=None, x2c=True, loc = False):
    from pyscf import cc, mp, lib, scf, df
    if x2c == True:
        mf = scf.rohf.ROHF(mol).x2c().density_fit()
    else:
...
def sie_rohf_uccsd_t_mp2_ao(mol, imp_idx, title, full_cc=False, eta=1e-2, x2c=True, loc=False):
    from pyscf import cc, mp, lib, scf, df
    if x2c == True:
        mf = scf.rohf.ROHF(mol).x2c().density_fit()
    else:
    ...
    return mymp2.e_tot, dmet_mp_ene, dmet_cc_ene, mymp2.e_corr + dmet_cc.e_corr + dmet_et - dmet_mp.e_corr + mydmet.es_e + mydmet.fo_ene
#### For LO(IC) #### 
def sie_rohf_uccsd_t_ic(mol, imp_idx, title, full_cc=False, x2c=True, frozen=False):
    from pyscf import cc, mp, lib, scf, df
    if x2c == True:
        mf = scf.rohf.ROHF(mol).x2c().density_fit()
    else:
    ...
    mf.level_shift = 2.0
    mf.conv_tol = 1e-8
    mf.max_memory =getattr(mol, 'max_memory', 200000)
    ...
    print(f"Cluster {title} CCSD(T) time: {t3 - t2:.2f} seconds")
    mydmet = uhf_tool.SSDMET_uhf(mf_uhf, title = f'DMET_{title}',  imp_idx = imp_idx,  es_method = 'eig', threshold = 1e-12).density_fit()
    mydmet.build(restore_imp = True, aodmet = False, save_chk = False)
...
def sie_rohf_uccsd_t_cl_ic(mol, imp_idx, title, full_cc=False, shell=None, atoms_A=None, proj_bas=None, x2c=True, loc = False):
    from pyscf import cc, mp, lib, scf, df
    if x2c == True:
        mf = scf.rohf.ROHF(mol).x2c().density_fit()
    else:
...
def sie_rohf_uccsd_t_mp2_ic(mol, imp_idx, title, full_cc=False, eta=1e-2, x2c=True, loc=False):
    from pyscf import cc, mp, lib, scf, df
    if x2c == True:
        mf = scf.rohf.ROHF(mol).x2c().density_fit()
    else:
