...
class SSDMET(lib.StreamObject):
    ...
    def calculate_bath_overlap_scores(self, ref_bath_coeff_ao, ref_mol=None):
        """
        计算当前结构的所有 Schmidt 环境轨道与参考结构 Bath 空间的重叠程度 (Fidelity/Similarity)。
        这是处理几何变化时选择一致性 Bath 的关键。
        
        Args:
            ref_bath_coeff_ao (ndarray): 参考结构的 Bath 轨道系数 (在参考结构 AO 基下)。
            ref_mol (gto.Mole): 参考结构的分子对象。
                                *关键*: 如果原子移动了，必须提供此参数来计算交叉重叠积分 S_cross。
                                如果为 None，则假设几何结构完全未变。
                                
        Returns:
            sim_scores (ndarray): 每个 Schmidt 环境轨道的重叠分数 (0.0 ~ 1.0)。
            evals_env (ndarray): 环境轨道的 Schmidt 占据数。
            evecs_env (ndarray): 环境轨道的系数（在 LO 基下的局部系数）。
        """
        if ref_mol is None:
            self.log.warn("calculate_bath_overlap_scores: ref_mol not provided. Assuming geometry is IDENTICAL (S_cross = S).")
            S_cross = self.mol.intor_symmetric('int1e_ovlp')
        else:
            # 计算两个不同几何结构基函数之间的交叉重叠 <chi_mu(R_new) | chi_nu(R_old)>
        ...
        # 这里使用 restore_imp=True 确保 Impurity 和 Environment 的定义与标准 build 一致
        ldm, caolo, cloao = self.lowdin_orth(restore_imp=True) 
        
        nao = self.mol.nao
        env_idx = [x for x in range(nao) if x not in self.imp_idx]
        
        # 2. 生成当前结构的所有 Schmidt 环境轨道 (Environment Natural Orbitals)
        #    取环境密度块 -> 对角化
        ldm_env = ldm[np.ix_(env_idx, env_idx)]
        evals_env, evecs_env = np.linalg.eigh(ldm_env)
        
        # 3. 将这些 Schmidt 轨道转换到当前 AO 基下，以便计算积分
        #    Schmidt 轨道 evecs_env 是在 Lowdin 基下的系数 (env部分)
        #    我们需要构造全尺寸矩阵 -> 左乘 caolo (LO->AO)
        lo_env_coeff = np.zeros((nao, len(env_idx)))
        lo_env_coeff[env_idx, :] = evecs_env
        eno_ao_curr = caolo @ lo_env_coeff
        
        # 4. 计算投影包含度 (Projection/Overlap Metric)
        #    ovlp_mat[i, j] = < Psi_env_i(new) | S_cross | Psi_bath_j(old) >
        #    dim: (N_env_curr, N_bath_ref)
        ovlp_mat = reduce(lib.dot, (eno_ao_curr.T.conj(), S_cross, ref_bath_coeff_ao))
        
        #    Score_i = Sum_j | <Psi_i | Psi_j> |^2
        #    物理意义：当前第 i 个环境轨道有多少成分属于参考 Bath 空间
        sim_scores = np.sum(np.abs(ovlp_mat)**2, axis=1)
        
        # 5. 打印详细结果
        self.log.info(f"{'='*20} Bath Consistency Analysis {'='*20}")
        self.log.info(f"Reference Bath Size: {ref_bath_coeff_ao.shape[1]}")
        self.log.info(f"Current Env Candidates: {len(sim_scores)}")
        self.log.info(f"{'Idx':>4} | {'Similarity':>10} | {'Schmidt Occ':>12} | {'Status'}")
        self.log.info("-" * 45)
        ...
        # 只是为了展示，设定一个简单的判定标准
        ref_size = ref_bath_coeff_ao.shape[1]
        
        for rank, idx in enumerate(sorted_indices):
            score = sim_scores[idx]
            occ = evals_env[idx]
            
            # 简单的标记：前 N 个最有可能是对应的 Bath
            status = "Bath Candidate" if rank < ref_size else "Frozen"
            if rank < ref_size + 5: # 只打印最有希望的前几名和边界附近的
                self.log.info(f"{idx:4d} | {score:10.6f} | {occ:12.6f} | {status}")
            
        self.log.info("-" * 45)
        self.log.info(f"Top {ref_size} avg score: {np.mean(sim_scores[sorted_indices[:ref_size]]):.4f}")
        self.log.info("="*65)
        
        return sim_scores, evals_env, evecs_env
        '''
    '''
    def calculate_bath_overlap_scores(self, ref_bath_coeff_ao, ref_mol=None):
        """
        ...
        # 使用类现有的 lowdin_orth 方法获取 Lo 基系数
        if self.dm is None:
            dm = mf_or_cas_make_rdm1s(self.mf_or_cas)
            if dm.ndim == 3: 
                self.dm = dm[0] + dm[1] # ROHF/UHF -> Total Density for Schmidt
                self.open_shell = True
            else:
                self.dm = dm
                self.open_shell = False
        ...
        # 转换回 AO 基底，用于计算积分
        # evecs_env_lo_sub 是 环境部分LO -> 环境特征轨道 的变换
        # 我们需要建立 全LO -> 全AO 的变换
        
        # 构造全尺寸的环境轨道系数矩阵 (在 LO 基下)
        # 每一列是一个环境轨道，行是 LO 索引
        N_env = len(env_idx)
        C_env_in_LO = np.zeros((nao, N_env))
        C_env_in_LO[env_idx, :] = evecs_env_lo_sub
        
        # 变换到 AO 基: C_AO = C_LO2AO @ C_LO
        ...
        if ref_mol is None:
            ...
            # 关键：计算两个不同几何结构基函数间的重叠积分
            s_cross = gto.intor_cross('int1e_ovlp', self.mol, ref_mol)
            
        # ref_bath_coeff_ao shape: (N_AO_ref, N_ref_bath)
        if ref_bath_coeff_ao is None:
             return np.zeros(N_env), evals_env, evecs_env_lo_sub

        # 计算重叠矩阵 O_ij = < Psi_env_i(new) | Psi_bath_j(ref) >
        #                   = C_env_i^T * S_cross * C_ref_j
        overlap_matrix = reduce(lib.dot, (evecs_env_ao.T.conj(), s_cross, ref_bath_coeff_ao))
        
        # 计算投影分数: Score_i = Sum_j |O_ij|^2
        sim_scores = np.sum(np.abs(overlap_matrix)**2, axis=1)
        
        # 返回 Schmidt 轨道在 LO 基下的系数 (Environment block part only) 供 build 使用
...
# ...existing code...0203
    ...
    def calculate_bath_overlap_scores(self, ref_bath_coeff_ao, ref_mol=None):
        """
        [Algorithm Update] 使用文献中的 Lowdin Cross-Overlap 方法计算相似度。
        公式: S_k,k+1 = (S^k)^(1/2) * (S^k+1)^(1/2)
        这实际上是在计算：如果基函数没有移动，仅仅是基函数的度规发生了变化时的等效重叠。
        这要求两个结构具有相同的原子顺序和基组定义。
        """
        # --- 0. 确保密度矩阵已初始化 ---
        if self.dm is None:
            dm = mf_or_cas_make_rdm1s(self.mf_or_cas)
        ...
        # 1. 获取当前结构的 Lowdin 变换矩阵
        # caolo_curr = (S^k+1)^(-1/2)
        # cloao_curr = (S^k+1)^(1/2) 
        ldm, caolo_curr, cloao_curr = self.lowdin_orth(restore_imp=True)
        self.caolo = caolo_curr
        self.cloao = cloao_curr

        # 2. 获取 Schmidt 环境轨道 (在当前 Lowdin 基下)
        nao = self.mol.nao
        env_idx = [x for x in range(nao) if x not in self.imp_idx]
        dm_env_lo = ldm[np.ix_(env_idx, env_idx)]
        evals_env, evecs_env_lo_sub = np.linalg.eigh(dm_env_lo)
        ...
        idx_sort = np.argsort(evals_env)[::-1]
        evals_env = evals_env[idx_sort]
        evecs_env_lo_sub = evecs_env_lo_sub[:, idx_sort] # Columns are Env Orbitals in LO basis

        if ref_bath_coeff_ao is None:
             return np.zeros(len(env_idx)), evals_env, evecs_env_lo_sub

        # 3. 构建 Cross Overlap Matrix (文献方法)
        # 目标: 计算 < Psi_env_i(curr) | Psi_bath_j(ref) >_effective
        # Psi_env(curr) = \sum C_LO_i(curr) * \chi_LO(curr)
        # Psi_bath(ref) = \sum C_AO_j(ref) * \chi_AO(ref)
        
        # 我们先将参考 Bath 变换到 "参考 Lowdin 基" 下。
        # 使得比较都在 Lowdin 空间进行。
        # 假设参考 Bath存储的是 AO 系数。
        
        # S_ref = S^k
        if ref_mol is None:
            s_ref = self.mol.intor_symmetric('int1e_ovlp')
        else:
            # 注意：如果 ref_mol 不同，我们需要计算 ref_mol 的 S 矩阵
            s_ref = ref_mol.intor_symmetric('int1e_ovlp')
        
        # 构造参考结构的 (S^k)^(1/2)
        # cloao_ref = (S^k)^(1/2)
        _, cloao_ref = lowdin_orth(self.mol, s_ref) 
        
        # 参考 Bath 在 Lowdin 基下的表达： C_LO_ref
        # C_AO = S^(-1/2) * C_LO => C_LO = S^(1/2) * C_AO
        # ref_bath_coeff_lo = (S^k)^(1/2) * C_AO(ref)
        ref_bath_coeff_lo = cloao_ref @ ref_bath_coeff_ao

        # 当前环境轨道在 Lowdin 基下的表达：evecs_env_lo_sub
        # (需要扩展到全维度以便点积，或者只比较环境部分，假设基函数一一对应)
        # 文献假设 "as though the basis had not moved"，这意味着我们直接比较系数向量
        # 就像比较两个一般向量一样，忽略基函数本身的空间差异。
        # 这种比较的前提是原子一一对应。
        
        # 构造全维系当前环境系数 C_LO_curr
        c_env_lo_curr = np.zeros((nao, len(env_idx)))
        c_env_lo_curr[env_idx, :] = evecs_env_lo_sub
        
        # 4. 计算 Lowdin 系数的直接重叠 (Direct Overlap of Coefficients)
        # O_ij = (C_LO_curr)^T * (C_LO_ref)
        # 这等价于：C_curr^T * (S^k+1)^-1/2 * (S^k+1)^1/2 * (S^k)^1/2 * (S^k)^-1/2 * C_ref ... 
        # 本质上是在度量两个 Lowdin 向量的相似形。
        
        # 但文献公式 (6) 实际上定义了一个度规张量 M = (S^k)^1/2 * (S^k+1)^1/2 ?
        # 不，通常文献中使用 Lowdin 正交化后，直接计算系数的点积 $C_k^T C_{k+1}$ 
        # 本质上就是忽略了基函数的 Cross-Overlap，而只看 (S^k)^(1/2) 和 (S^k+1)^(1/2) 带来的“刚性”变换。
        # 另外一种解释是：
        # Overlap approx = C_AO_curr^dag * S_eff * C_AO_ref
        # S_eff = (S_curr)^(1/2) * (S_ref)^(1/2)
        # 让我们按照这个 S_eff 公式来实施。
        
        # S_eff = cloao_curr @ cloao_ref = (S^k+1)^(1/2) * (S^k)^(1/2)  (顺序注意矩阵乘法)
        # 但矩阵乘法通常是对称处理或取平均。文献公式是标量形式还是矩阵？如果是矩阵：
        S_eff = cloao_curr @ cloao_ref # (S_curr)^1/2 * (S_ref)^1/2
        
        # 计算 AO 系数间的重叠
        # 获取当前环境轨道的 AO 系数
        evecs_env_ao = caolo_curr @ c_env_lo_curr
        
        # Overlap Matrix = C_env(curr)^T * S_eff * C_bath(ref)
        # 代入：
        # = (C_lo_curr^T * S_curr^-1/2) * (S_curr^1/2 * S_ref^1/2) * (S_ref^-1/2 * C_lo_ref)
        # = C_lo_curr^T * I * C_lo_ref
        # = C_lo_curr^T * C_lo_ref
        
        # 结论：如果您想要使用公式 (S^k)^1/2 (S^k+1)^1/2 作为度规，
        # 在数学上等价于直接计算**Lowdin 基下系数的点积**。
        # 这正是 "Comparing orbitals as though the basis had not moved" 的数学含义。
        
        overlap_matrix_lo = np.dot(c_env_lo_curr.T, ref_bath_coeff_lo)
        
        # 计算相似度
        sim_scores = np.sum(np.abs(overlap_matrix_lo)**2, axis=1)
    ...
    def build_union_with_reference(self, ref_bath_coeff_ao, ref_mol=None):
        ...
        # evecs_env_sub 是 LO 基下仅包含环境部分的系数
        ...
        M = ref_bath_coeff_ao.shape[1] # 参考 Bath 的大小
        # 排序索引 (分数从大到小)
        ...
        # 优化：如果分数太低 (比如 < 0.1)，说明即使是 Top M 也没什么物理对应关系，可以过滤
        # 但为了保证“M个”的数量，暂时不强力过滤，相信并集策略
        
        # 3. 选取 S_std: Threshold
        # evals_env 已经是 0~2 的占据数
        ...
        self.log.info(f"{'='*20} Union Bath Strategy {'='*20}")
        self.log.info(f"Ref Size M={M}, Std Size={len(set2_indices)}")
        self.log.info(f"Set Ref (Top-M): {sorted(set1_indices)}")
        self.log.info(f"Set Std (Thres): {sorted(set2_indices)}")
        ...
        # 6. 组装最终的变换矩阵 lo_cloes [Imp | Bath | FO | FV]
        # Imp 部分: 单位阵的相应列
        c_imp = np.eye(nao)[:, self.imp_idx]
        
        # 辅助函数：Embedding (N_env_sub x N_cols) -> (N_total_LO x N_cols)
        ...
        # [Fix] 正确设置 es_occ，防止 make_es_dm 报错
        # 我们可以根据杂质和 Bath 的占据数来构造
        # Impurity occupancy can be approximated or retrieved from ldm
        # Environment (Bath) occupancy comes from evals_env
        if self.es_natorb:
            # Re-diagonalize the embedded space density matrix to get natural orbitals and occupation
            # Construct ES density in the current provisional basis
            # D_es = C_es^T @ D_lo @ C_es
            ldm_full = reduce(lib.dot, (self.cloao, self.dm, self.cloao.conj().T))
            C_es_prov = self.lo_cloes[:, :self.nes] # [Imp | Bath]
            D_es_prov = reduce(lib.dot, (C_es_prov.conj().T, ldm_full, C_es_prov))
            
            # Diagonalize D_es to update C_es and get es_occ
            occ_es, u_es = np.linalg.eigh(D_es_prov)
            
            # Sort descending
            idx_sort = np.argsort(occ_es)[::-1]
            self.es_occ = occ_es[idx_sort]
            u_es = u_es[:, idx_sort]
            
            # Rotate the ES orbitals to natural orbital basis
            # Update lo_cloes columns for ES
            C_es_natural = C_es_prov @ u_es
            self.lo_cloes[:, :self.nes] = C_es_natural
            
            # Note: This mixes Imp and Bath orbitals, which is standard for es_natorb=True
            # If you want to keep Imp/Bath separated (Schwarz method), set es_natorb=False
        else:
            # If not using natural orbitals, we must provide dummy es_occ or exact diagonal
            # For make_es_dm logic (which uses occupation thresholds), we need reasonable values.
            # But usually es_natorb=False means we use D matrix directly in make_es_dm.
            self.es_occ = None # make_es_dm handles None if es_natorb is False
            
            # Wait, make_es_dm logic for es_natorb=False:
            # if not self.es_natorb: es_dm = reduce(...) without es_occ
            # So this branch is fine if es_natorb is False.
            
            # If user explicitly set es_natorb=True, we MUST provide self.es_occ.
            pass
        # 8. 重建积分
...
# ...existing code...
    ...
    def append_bath_by_env_idx(self, env_idx):
        """
        手动将当前环境块中的指定列追加为新的 bath 轨道。
        同时正确更新电子数（通过更新 self.nfo 和 self.nfv）。
        """
        if self.lo_cloes is None or self.caolo is None or self.cloao is None:
            raise RuntimeError("Embedded subspace not built or transforms not cached. Run build() first.")
        ...
        # 1. 识别当前环境的划分情况
        # 环境块（LO 基）在 lo_cloes 中的列索引范围
        idx_start_env = nimp + nbath
        # 当前环境块分为 [FO | FV]
        # FO 范围相对 env_block: 0 ~ self.nfo
        # FV 范围相对 env_block: self.nfo ~ self.nfo + self.nfv
        
        # 2. 统计被移走的 FO 和 FV 数量
        # env_idx 是用户传入的、相对于整个环境块 [FO | FV] 的索引列表
        n_shifted_fo = 0
        n_shifted_fv = 0
        
        # 注意：这里的逻辑假设 env_idx 是相对于当前 self.lo_cloes 中环境部分的索引
        # 但要注意 build() 里面原本的布局是 [Imp | Bath | FO | FV]
        # 如果 env_idx 是 append_bath_by_env_idx 的输入，我们需要知道它指向的是 FO 还是 FV
        
        # 很多时候 env_idx 可能是通过 find_bath_indices... 得到的，我们需要小心处理
        # 简单的做法是：不用复杂的投影判断，直接基于索引位置判断
        
        # 修正：我们需要先获取环境块的轨道矩阵，然后看 idx 落在哪里
        # Build 之后的 lo_cloes 结构: [Imp | Bath | FO | FV]
        # 环境部分从 nimp+nbath 开始
        
        # 3. 提取轨道并重组
        Q_emb = self.lo_cloes[:, :nimp+nbath]
        env_block = self.lo_cloes[:, nimp+nbath:] # 这里就是 [FO, FV]
        
        # 确定新 Bath 和剩余环境
        # 这里的 env_idx 必须是相对于 env_block 的索引 (0 ~ nfo+nfv-1)
        
        # 4. 关键修正：计算电子数的变化
        # 如果 idx < self.nfo，说明我们在移走一个 Frozen Occupied 轨道
        # 这意味着这个电子对进入了活性空间，Frozen Core 电子数减少，嵌入体系电子数增加
        for idx in env_idx:
            if idx < self.nfo:
                n_shifted_fo += 1
            else:
        ...
        # 5. 执行轨道移动 (与原代码类似，但更严谨)
        # 用 mask 分离选中的和剩余的
        mask_move = np.zeros(env_block.shape[1], dtype=bool)
        mask_move[env_idx] = True
        
        B_new_bath = env_block[:, mask_move] # 选中的列
        C_remain = env_block[:, ~mask_move]  # 剩余的列
        
        # 为了数值稳定性，对新 Bath 正交化（对应原代码的 svd(P @ Bcand)）
        # 这里其实 B_new_bath 本身就是正交的（来自 lo_cloes），且与 Q_emb 正交
        # 所以直接用即可，或者做个 QR 保证精度
        lo2New_bath, _ = np.linalg.qr(B_new_bath)
        
        # 剩余环境保持原样即可，不需要重新混合 core/vir，因为我们是根据索引精确挑选的
        # 原来的 FO 剩下的还是 FO，原来的 FV 剩下的还是 FV
        # 我们不能像原代码那样重新做 QR 混在一起，否则会丢失占据信息
        
        # 分离剩余的 FO 和 FV
        # 原来的 env_block 是 [FO (0:nfo) | FV (nfo:)]
        # 我们构建两个 mask
        mask_fo = np.zeros(env_block.shape[1], dtype=bool)
        mask_fo[:self.nfo] = True
        
        mask_fv = np.zeros(env_block.shape[1], dtype=bool)
        mask_fv[self.nfo:] = True
        
        # 剩余 FO = (原有 FO) AND (未被选中)
        mask_remain_fo = mask_fo & (~mask_move)
        # 剩余 FV = (原有 FV) AND (未被选中)
        mask_remain_fv = mask_fv & (~mask_move)
        
        lo2New_core = env_block[:, mask_remain_fo]
        lo2New_vir  = env_block[:, mask_remain_fv]
        
        # 6. 更新 lo_cloes 和 AO 轨道
        self.lo_cloes = np.hstack([Q_emb, lo2New_bath, lo2New_core, lo2New_vir])
        
        # 7. 更新计数
        self.nes  = nimp + nbath + lo2New_bath.shape[1] # 嵌入空间变大
        self.nfo -= n_shifted_fo # 冻结占据减少 -> 嵌入电子数增加
        self.nfv -= n_shifted_fv # 冻结虚轨减少
        ...
        # 9. 重新生成 MF 对象
        # 请注意：ROHF() 初始化时会读取 self.nfo 来计算嵌入体系的 nelectron
        # nelectron = total - 2 * self.nfo
        # 因为 self.nfo 减小了，nelectron 会自动增加，从而修正了电荷问题
        self.es_mf = self.ROHF()
...
# ...existing code...
    def append_bath_by_env_idx(self, env_idx):
        """
        手动将当前环境块中的指定列追加为新的 bath 轨道。
        env_idx: List[int]，相对于当前 LO 基 Environment 块 ([Bath | FO | FV]) 的列索引。
                 通常 indices < nbath 的部分表示已有 Bath，会被忽略。
                 indices >= nbath 的部分会被移动到 Bath 中。
        """
        if self.lo_cloes is None or self.caolo is None or self.cloao is None:
            raise RuntimeError("Embedded subspace not built or transforms not cached. Run build() first.")
        ...
        # 1. 过滤索引并进行坐标转换
        # 用户(或搜索函数)传入的 env_idx 通常是相对于 [Bath | FO | FV] 的。
        # 我们需要将其转换为相对于 [FO | FV] 块的局部索引 (即减去 nbath)。
        
        indices_to_move = [] # 存储相对于 [FO|FV] 块的索引
        for idx in env_idx:
            if idx < nbath:
        ...
        # 这里的 local_idx 是相对于 env_block (FO开头) 的
        for local_idx in indices_to_move:
            # env_block 的前 nfo 个是 Frozen Occupied
...
# ...existing code...
    ...
    def ccsdt_solver(self, with_t=True):
        ...
        return final_e_tot
    ''' old version ccsd(t) solver 2026.1.28
    def ccsdt_solver(self, with_t=True):
        " run ccsd(t) as the solver for dmet subspace "
        from pyscf import cc
        if self.mol.spin != 0:
