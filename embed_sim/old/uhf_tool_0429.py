...
class SSDMET_uhf:
    ...
    def _fo_ene(self):
        ...
        vhf0, vhf1 = vj0 + vj1 - vk0, vj0 + vj1 - vk1

        e1 = np.einsum('ij,ji->', hcore, dm_fo0) + np.einsum('ij,ji->',hcore,dm_fo1)
        e2 = 0.5 * (np.einsum('ij,ji->', vhf0, dm_fo0) + np.einsum('ij,ji->', vhf1, dm_fo1))
        e_elec = e1 + e2
        fo_ene = e_elec + self.mf_or_cas.energy_nuc()
