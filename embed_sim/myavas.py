...
def kernel(mf, aolabels, threshold=THRESHOLD, minao=MINAO, with_iao=WITH_IAO,
    ...
    avas_obj = AVAS(mf, aolabels, threshold, minao, with_iao,
                    openshell_option, canonicalize, ncore, nunocc, verbose)
...
def _kernel(avas_obj):
    ...
    nunocc = avas_obj.nunocc
    nunfreeze = numpy.shape(mo_coeff)[0]-nunocc
    ...
    if avas_obj.with_iao:
        ...
        s21 = reduce(numpy.dot, (c.T, ovlp, mo_coeff[:, ncore:nunfreeze]))
    else:
        s2 = pmol.intor_symmetric('int1e_ovlp')[baslst][:,baslst]
        s21 = gto.intor_cross('int1e_ovlp', pmol, mol)[baslst]
        s21 = numpy.dot(s21, mo_coeff[:, ncore:nunfreeze])
    ...
    if avas_obj.openshell_option == 2:
        ...
        mocas = numpy.hstack((mocas,
                              mo_coeff[:,nocc:nunfreeze].dot(u[:,wvir>=threshold])))
        movir = mo_coeff[:,nocc:nunfreeze].dot(u[:,wvir<threshold])
        ncas = mocas.shape[1]
    ...
    mofreeze_unocc = mo_coeff[:,nunfreeze:]
    if avas_obj.canonicalize:
        ...
        if nunocc > 0:
            mofreeze_unocc = trans(mofreeze_unocc)
        mocore = trans(mocore)
        mocas = trans(mocas)
        movir = trans(movir)
    mo = numpy.hstack((mofreeze, mocore, mocas, movir, mofreeze_unocc))
...
class AVAS(lib.StreamObject):
    def __init__(self, mf, aolabels, threshold=THRESHOLD, minao=MINAO,
                 ...
                 canonicalize=CANONICALIZE, ncore=0, nunocc=0, verbose=None):
        self._scf = mf
