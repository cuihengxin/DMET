...
def _mem_log(log, tag):
    rss, vms = lib.current_memory()
    ru_maxrss = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
    peak = ru_maxrss/1e3 if sys.platform.startswith('linux') else ru_maxrss/1e6
    log.info('MEM  %-16s RSS %10.1f MB   VMS %10.1f MB   peakRSS %10.1f MB',
             tag, rss, vms, peak)
...
def get_RMP2_bath(mf, es_mf, ao2eo, ao2core, ao2vir, lo2core, lo2vir, eta=1e-4, verbose=None):
    ...
    log.info('')
    log.info(f"**** Time Summary")
    log.info(f"Initial time, {t1-t0}")
    log.info(f"eris_Ov time, {t2-t1} ")
    log.info(f"eris_oV time, {t3-t2} ")
    log.info(f"t_ijab, {t4-t3}")
    log.info(f"D ijab, {t5-t4}")
    log.info(f"D mp2 core vir, {t6-t5}")
    log.info(f"Total time {t6-t0}")
    return lo2MP2_bath, lo2MP2_core, lo2MP2_vir
def get_RMP2_bath_sos(mf, es_mf, ao2eo, ao2core, ao2vir, lo2core, lo2vir, eta=1e-4, verbose=None):
    ...
    def _gamma1_intermediates(mf, t2=None, eris=None):
        ...
        for i in range(nocc):
            t2i = t2[i]
            l2i = t2i
            '''log.info('get DM1 with full MP2 amplitudes')
                dm1vir += lib.einsum('jca,jcb->ba', l2i, t2i) * 2 \
                        - lib.einsum('jca,jbc->ba', l2i, t2i)
                dm1occ += lib.einsum('iab,jab->ij', l2i, t2i) * 2 \
                        - lib.einsum('iab,jba->ij', l2i, t2i)'''
            log.info('get DM1 with J-only MP2 amplitudes')
            dm1vir += lib.einsum('jca,jcb->ba', l2i, t2i) * 2
            dm1occ += lib.einsum('iab,jab->ij', l2i, t2i) * 2
        dm1occ *= -1
        dm1vir += dm1vir.T
...
def get_UMP2_bath(mf, es_mf, ao2eo, ao2core, ao2vir, lo2core, lo2vir, eta=1e-2, verbose=None):
    ...
    log.info('')
    log.info(f"**** Time Summary")
    log.info(f"eris time, {t1-t0} ")
    log.info(f"t2 time, {t2-t1} ")
    log.info(f"gamma1 time, {t3-t2} ")
    log.info(f"eigenvalue time, {t4-t3} ")
    log.info(f"Total time {t4-t0}")
    return lo2MP2_bath, lo2MP2_core, lo2MP2_vir
def get_UMP2_bath_sos(mf, es_mf, ao2eo, ao2core, ao2vir, lo2core, lo2vir, eta=1e-2, verbose=None):
...
def get_ROMP2_bath(mf, es_mf, ao2eo, ao2core, ao2vir, lo2core, lo2vir, ao = False, readmp2 = False, eta=1e-2, verbose=None):
    ...
    if readmp2:
        ...
        if eta < 1:
            ...
            _mem_log(log, 'semi-canonicalize')
            ...
            log.info('')
            t4 = lib.logger.perf_counter()
            log.info('Time for constructing ROMP2 bath: %.2f sec (%.2f sec for ERIs, %.2f sec for t1/t2, %.2f sec for DM1, %.2f sec for DM1->AO->BNO)', t4-t0, t1-t0, t2-t1, t3-t2, t4-t3)
            _mem_log(log, 'end')
    return lo2MP2_bath, lo2MP2_core, lo2MP2_vir

def get_ROMP2_bath_sos(mf, es_mf, ao2eo, ao2core, ao2vir, lo2core, lo2vir, ao = False, readmp2 = False, eta=1e-2, verbose=None):
    log = logger.new_logger(mf, verbose=verbose)
    if readmp2:
        log.info('========== Read from MP2.TXT files ===========')
        if eta < 1:
        ...
        if eta < 1:
            ...
            def get_t2(mf, occ_energy=None, vir_energy=None, eris=None, with_t2=True, verbose=None):
                ...
                if with_t2:
                    t2 = np.zeros((nocc[0],nocc[1],nvir[0],nvir[1]), dtype=eris.dtype)
                    t2_ptr = t2.ctypes.data_as(ctypes.c_void_p)
                    mem_avail -= t2.size * eris.dsize / 1e6
                else:
                    t2 = None
            ...
            def _gamma1_intermediates(mf, t1=None, t2=None, eris=None):
                ...
                t1a, t1b = t1
                t2ab = t2
                nocca, noccb, nvira, nvirb = t2.shape
