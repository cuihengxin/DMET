...
echo "start time: $(date +"%Y-%m-%d %H:%M:%S")" >> consistent_es_h2o2.log
start_ts=$(date +%s)
#module load icc/latest
#module load mkl/32/2023.2.0
#source activate mokit-py39
python consistent_es_h2o2.py >consistent_es_h2o2.log

end_ts=$(date +%s)
echo "end time: $(date +"%Y-%m-%d %H:%M:%S")" >> consistent_es_h2o2.log
...
if date --version >/dev/null 2>&1; then
    printf "elapsed: %s\n" "$(date -ud "@$elapsed" +'%H:%M:%S')" >> consistent_es_h2o2.log
else
    # 备选：手工计算 H:M:S
    days=$((elapsed/86400)); hh=$(( (elapsed%86400)/3600 )); mm=$(( (elapsed%3600)/60 )); ss=$((elapsed%60))
    if [ $days -gt 0 ]; then
        printf "elapsed: %dd %02d:%02d:%02d\n" $days $hh $mm $ss >> consistent_es_h2o2.log
    else
        printf "elapsed: %02d:%02d:%02d\n" $hh $mm $ss >> consistent_es_h2o2.log
    fi
fi
