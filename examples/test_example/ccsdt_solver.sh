...
#SBATCH -n 16
##SBATCH -c 16
#SBATCH -o ccsdt_solver.out
#SBACTH -e ccsdt_solver.err
##SBATCH -w node06
##SBATCH --mem=32GB
...
echo "start time: $(date +"%Y-%m-%d %H:%M:%S")" >> ccsdt_solver.log
start_ts=$(date +%s)
#module load icc/latest
#module load mkl/32/2023.2.0
source activate mokit-py39
python ccsdt_solver.py >ccsdt_solver.log

end_ts=$(date +%s)
echo "end time: $(date +"%Y-%m-%d %H:%M:%S")" >> ccsdt_solver.log
