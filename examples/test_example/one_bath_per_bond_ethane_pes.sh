...
python one_bath_per_bond_ethane_pes.py >one_bath_per_bond_ethane_pes.log
...
if date --version >/dev/null 2>&1; then
    ...
    if [ $days -gt 0 ]; then
        printf "elapsed: %dd %02d:%02d:%02d\n" $days $hh $mm $ss >> one_bath_per_bond_ethane_pes.log
    else
        printf "elapsed: %02d:%02d:%02d\n" $hh $mm $ss >> one_bath_per_bond_ethane_pes.log
    fi
fi
