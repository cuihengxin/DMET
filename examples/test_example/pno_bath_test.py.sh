...
echo "start time: $(date +"%Y-%m-%d %H:%M:%S")" >> pno_bath_test.py.log
start_ts=$(date +%s)
#module load icc/latest
#module load mkl/32/2023.2.0
#source activate mokit-py39
python pno_bath_test.py.py >pno_bath_test.py.log

end_ts=$(date +%s)
echo "end time: $(date +"%Y-%m-%d %H:%M:%S")" >> pno_bath_test.py.log
