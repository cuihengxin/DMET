...
mf.kernel()
imp = mol.search_ao_label('Co 3d')
print("impurity orbitals: ", imp)
imp2 = mol.search_ao_label('Co.*')
imp3 = mol.search_ao_label('Co 3d')
print("impurity orbitals: ", imp2)
mydmet = ssdmet.SSDMET(mf, title=title, imp_idx=imp, bath_option={'ROMP2':43},es_natorb=False).density_fit()
# if impurity is not assigned, the orbitals on the first atom is chosen as impurity
mydmet.build(iaopao='IAOPAO',restore_imp =True, save_chk=False, ip_iao=None, imp4ip=imp3)
