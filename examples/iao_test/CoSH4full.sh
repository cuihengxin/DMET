...
echo "start time: $(date +"%Y-%m-%d %H:%M:%S")" >> CoSH4full.log
start_ts=$(date +%s)
#module load icc/latest
#module load mkl/32/2023.2.0
#source activate mokit-py39
python CoSH4full.py >CoSH4full.log

end_ts=$(date +%s)
echo "end time: $(date +"%Y-%m-%d %H:%M:%S")" >> CoSH4full.log
