...
echo "start time: $(date +"%Y-%m-%d %H:%M:%S")" >> iao_test_ip.log
start_ts=$(date +%s)
#module load icc/latest
#module load mkl/32/2023.2.0
#source activate mokit-py39
python iao_test_ip.py >iao_test_ip.log

end_ts=$(date +%s)
echo "end time: $(date +"%Y-%m-%d %H:%M:%S")" >> iao_test_ip.log
