...
class SSDMET_uhf:
    ...
    def lowdin_orth(self, restore_imp = False):
        """Lowdin orthonormalize DM separately for alpha and beta spins.

        Returns
        -------
        ldm_a, ldm_b : ndarray
            Spin-resolved DM in LO basis.
        caolo_a, caolo_b : ndarray
            S^{-1/2} per spin.
        cloao_a, cloao_b : ndarray
            S^{1/2} per spin.
        """
        S = self.mf_or_cas.get_ovlp()
        # Shared initial LO basis (S^{-1/2} is spin-independent)
        caolo0 = lowdin(S)
        cloao0 = caolo0 @ S
        ...
        for s in range(2):
            ...
            caolo_s = caolo0.copy()
            cloao_s = cloao0.copy()
            print(f"Performing Lowdin orthogonalization for spin {s} with impurity index {self.imp_idx} and threshold {self.threshold}", "LOIC operation")
            if restore_imp:
                imp_idx = self.imp_idx
                mask_env = np.ones(len(caolo_s), dtype=bool)
                mask_env[imp_idx] = False

                Q1 = cloao_s[:, imp_idx]
                Q1, _ = np.linalg.qr(Q1)
                P = np.eye(*cloao_s.shape) - Q1 @ Q1.T.conj()
                B = P @ cloao_s[:, mask_env]
                from scipy.linalg import svd
                U, S_svd, Vh = svd(B, full_matrices=False)

                Q = np.zeros(cloao_s.shape)
                Q[:, imp_idx] = Q1
                Q[:, mask_env] = U[:, 0: cloao_s.shape[0] - len(imp_idx)]
                cloao_s = Q.T.conj() @ cloao_s
                caolo_s = caolo_s @ Q

            ldm_s = cloao_s @ dm_s @ cloao_s.conj().T
            ldm_out.append(ldm_s)
            caolo_out.append(caolo_s)
            cloao_out.append(cloao_s)

        return (ldm_out[0], ldm_out[1],
                caolo_out[0], caolo_out[1],
                cloao_out[0], cloao_out[1])
    ...
    def build(self, restore_imp=False, aodmet = False, chk_fname_load='', save_chk=True):
        ...
        if not loaded:
            if aodmet:
                ...
                ldm0, ldm1, caolo_a, caolo_b, cloao_a, cloao_b = self.lowdin_orth(restore_imp=restore_imp)
            ...
            if restore_imp:
                self.caolo = (caolo_a, caolo_b)
                self.cloao = (cloao_a, cloao_b)
                self.caoes = (caolo_a @ cloes0, caolo_b @ cloes1)
            else:
                self.caolo = caolo
                self.cloao = cloao
                self.caoes = (caolo @ cloes0, caolo @ cloes1)
            self.lo_cloes = (cloes0, cloes1)
            self.log.info(f"Caolo shape {self.caolo[0].shape}, Cloao shape {self.cloao[0].shape}, Cloes shape {self.caoes[0].shape}")
    ...
    def density_fit(self, with_df=None):
        ...
        from uhf_dmet_ic.df_uhf_tool import DFSSDMET_uhf
